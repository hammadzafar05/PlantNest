...
$("#changePassw").on('click',()=>{
    ...
    if(oldP==""||newP==""||conP==""){
        ...
        $.ajax({
            "type":"POST",
            "url":baseURL+"thrift/check-password",
            "data":{
                "_token":csrf,
                "user":userId,
                "oldP":oldP,
            ...
            success:function(res){
                    ...
                    if(res==1){
                        ...
                        if(newP!=conP){
                            ...
                            $("#newP ,#conP").val('')
                            alertBox("Please Make sure that your both passwords match" ,"alertMsg2" ,"0")
                            ...
                            $.ajax({
                                "type":"POST",
                                "url":baseURL+"thrift/update-password",
                                "data":{
                                    "_token":csrf,
                                    "newPassword":newP,
                                },
                                success:function(res){
                                    if(res){
                                        $("#oldP, #newP ,#conP").val('')
                                        alertBox("Your Password has been updated successfully" ,"alertMsg2" ,"1")
                                    }else{
                                        $("#oldP").val('')
                                        alertBox("Something went wrong updating your password" ,"alertMsg2" ,"0")
                                    }
                                }
                            })
                        ...
                        $("#oldP").val('')
                        alertBox("Your Old Password is incorrect" ,"alertMsg2" ,"0")
